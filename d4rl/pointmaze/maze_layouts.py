...
def sample_layout(seed=None, size=20, max_len_frac=0.5, min_len_frac=0.3, coverage_frac=0.25, temp=20):
    """
...
def layout2str(layout, goal_exist, chaser_exist, box=None):
    """Transfers a layout matrix to string format that is used by MazeEnv class."""
    h, w = layout.shape
    padded_layout = np.ones((h + 2, w + 2))
    padded_layout[1:-1, 1:-1] = layout
    ...
    if goal_exist:
        output_str = re.sub("O", "G", output_str, count=1)  # add goal at random position
    if chaser_exist:
        output_str = re.sub("O", "C", output_str, count=1)  # add chaser at random position
    if box:
        for i in range(box):
            output_str = re.sub("O", "B", output_str, count=1)  # add box at random position
    return output_str


def rand_layout(seed=None, goal_exist=True, chaser_exist=False, box=None, **kwargs):
    """Generates random layout with specified params (see 'sample_layout' function)."""
    rand_layout = sample_layout(seed, **kwargs)
    layout_str = layout2str(rand_layout, goal_exist, chaser_exist, box)
    return layout_str
